...
class Glitter7engine {
  constructor(canvas, config = {}) {
    ...
    // Heightmap
    this.heightMap = null;
    this.HEIGHT_SCALE = config.heightScale || 1.0;
  ...
  initTextures() {
    ...
    this.heightMapTexture = this.gl.createTexture();
    ...
    // Heightmap texture
    this.gl.bindTexture(this.gl.TEXTURE_2D, this.heightMapTexture);
  ...
  getSkydomeFS() {
    ...
    void main() {
      ...
      v = v * ${this.skydome_vert_ajust} + 0.15;
...
getGroundFS() {
    ...
    void main() {
      ...
      // Primera aproximación para obtener la posición del heightmap
      ...
      vec2 heightTexCoord = vec2(worldX / ${this.MAP_WIDTH}.0, worldY / ${this.MAP_HEIGHT}.0);
      float terrainHeight = texture(u_heightMapTexture, heightTexCoord).r * u_heightScale;
      
      // Recalcular perspective considerando la altura del terreno
      perspective = (u_camera.z - terrainHeight) / (screenY - horizon);
      
      if (perspective < 0.0 || perspective > ${this.MAX_RENDER_DISTANCE}.0) {
        discard;
      }
      worldX = u_camera.x + (cosA * dx - sinA * 1.0) * perspective;
worldY = u_camera.y + (sinA * dx + cosA * 1.0) * perspective;
      ...
      float tx = float(floor(worldX));
      float ty = float(floor(worldY));
  ...
  getBillboardVS() {
    ...
    out vec2 v_texCoord;
    void main() {
      vec2 pos = u_screenPos + vec2(a_offset.x, a_offset.y + 1.0 + u_heightOffset) * u_size;
  ...
  getBlockVS() {
    ...
    void main() {
      ...
      // Obtener altura del terreno
      vec2 heightTexCoord = vec2(instancePos.x / ${this.MAP_WIDTH}.0, instancePos.z / ${this.MAP_HEIGHT}.0);
      float terrainHeight = texture(u_heightMapTexture, heightTexCoord).r * u_heightScale;
  ...
  createSkydomeGeometry() {
    ...
    for (let ring = 0; ring <= rings; ring++) {
      const phi = (ring / rings) * Math.PI * 0.95;
  ...
  initUniforms() {
    this.uniformLocations = {
      ...
      ground: {
        ...
        heightMapTexture: this.gl.getUniformLocation(this.program, 'u_heightMapTexture'),
...
heightOffset: this.gl.getUniformLocation(this.billboardProgram, 'u_heightOffset')
},
block: {
...
heightMapTexture: this.gl.getUniformLocation(this.blockProgram, 'u_heightMapTexture'),
heightScale: this.gl.getUniformLocation(this.blockProgram, 'u_heightScale')
},
sky: {
...
const data = new Float32Array(this.MAP_WIDTH * this.MAP_HEIGHT);
...
this.gl.bindTexture(this.gl.TEXTURE_2D, this.heightMapTexture);
this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.R32F, this.MAP_WIDTH, this.MAP_HEIGHT, 0, this.gl.RED, this.gl.FLOAT, data);
this.gl.pixelStorei(this.gl.UNPACK_FLIP_Y_WEBGL, true);
}
...
const index = tileY * this.MAP_WIDTH + tileX;
return this.heightMap[index] * this.HEIGHT_SCALE;
}
setTileMap(tileMap) {
...
const terrainHeight = this.getTerrainHeight(worldX, worldY);

const horizon = 0.3;
const invRotY = 1.0 / (rotY - 0.001);
const perspective = (camera.z - terrainHeight) * invRotY;
...
const horizon = 0.3;
const invRotY = 1.0 / (rotY - 0.001);
const perspective = (camera.z - (height + terrainHeight)) * invRotY;
...
this.gl.uniform1f(this.uniformLocations.ground.heightScale, this.HEIGHT_SCALE);
...
this.gl.bindTexture(this.gl.TEXTURE_2D, this.heightMapTexture);
this.gl.uniform1i(this.uniformLocations.ground.heightMapTexture, 2);
...
const heightOffset = billboard.proj.terrainHeight || 0;
...
this.gl.uniform1f(this.uniformLocations.billboard.heightOffset, heightOffset * billboard.proj.size * 0.1);
...
this.gl.uniform1f(this.uniformLocations.block.heightScale, this.HEIGHT_SCALE);
...
this.gl.activeTexture(this.gl.TEXTURE1);
this.gl.bindTexture(this.gl.TEXTURE_2D, this.heightMapTexture);
this.gl.uniform1i(this.uniformLocations.block.heightMapTexture, 1);
